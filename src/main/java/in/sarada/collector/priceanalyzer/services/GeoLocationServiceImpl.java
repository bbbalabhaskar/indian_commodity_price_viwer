...
import in.sarada.collector.priceanalyzer.repos.LocationInfoRepository;
import in.sarada.collector.priceanalyzer.shared.LocationInfo;
...
public class GeoLocationServiceImpl implements GeoLocationService {
    ...
    private Logger logger = LoggerFactory.getLogger(GeoLocationServiceImpl.class);

    @Override
    public Iterable<in.sarada.collector.priceanalyzer.model.LocationInfo> getAllLocations() {
        return locationInfoRepository.findAll();
    }

    @Override
    public Iterable<in.sarada.collector.priceanalyzer.model.LocationInfo> getByMarket(String market) {
        return locationInfoRepository.findAllByMarket(market);
    }
    ...
    public void initLocationCollection() {
        ...
        for (String market : markets) {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(String.format(opencageDataUrl, URLEncoder.encode(market, StandardCharsets.UTF_8))))
            ...
            httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                    ...
                    .thenAccept(s -> {
                        if (!StringUtils.isEmpty(s)) {
                            try {
                                LocationInfo info = objectMapper.readValue(s, LocationInfo.class);
                                saveLocation(market, info);
                            } catch (IOException e) {
                                logger.error("Exception in getting location data", e);
                            }
                        }
                    })
                    .join();
    ...
    private void saveLocation(String market, LocationInfo info) throws JsonProcessingException {
        if (info != null && info.getResults() != null && info.getResults().size() > 0) {
            in.sarada.collector.priceanalyzer.model.LocationInfo dbRefLocationInfo = new in.sarada.collector.priceanalyzer.model.LocationInfo();
            dbRefLocationInfo.setLat(info.getResults().get(0).getGeometry().getLat());
            dbRefLocationInfo.setLng(info.getResults().get(0).getGeometry().getLat());
            dbRefLocationInfo.setMarket(market);
            logger.info("Saved: {}", objectMapper.writeValueAsString(locationInfoRepository.save(dbRefLocationInfo)));
        }
    }
